        ...
        {shown.map((gw) => (
          <section key={gw.id}>
            <div className="flex items-baseline gap-3 mb-2">
              <h3 className="font-display text-lg">Kolo {gw.number}</h3>
            ...
            {gw.matches.length === 0 ? (
              <p className="text-slate-400 text-sm bg-navy-800 rounded-xl px-4 py-3">
                Za ovo kolo još nema uvezenih utakmica.
              </p>
            ) : (
    ...
    <li className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 bg-navy-800 px-3 py-2.5 sm:px-4">
      ...
      <span className="text-center min-w-[76px]">
        {played ? (
          ...
          <span className="text-danger-400 text-xs">{STATUS_LABEL[m.status]}</span>
        ) : (
          <span className="text-slate-300 text-sm tabular-nums">
            {kickoff.toLocaleTimeString("sr-RS", { hour: "2-digit", minute: "2-digit" })}
          </span>
        )}
        <span className="block text-[10px] text-slate-500 mt-0.5">
          {m.status === "live"
            ? STATUS_LABEL.live
            : kickoff.toLocaleDateString("sr-RS", { day: "numeric", month: "short" })}
        </span>
      </span>
