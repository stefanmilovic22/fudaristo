...
export function SquadBuilder({
  ...
  function blockedReason(p: SelectablePlayer): string | null {
    ...
    if (countByClub(p.club_id) >= MAX_PLAYERS_PER_CLUB) return "Već 3 iz kluba";
  ...
  function resetSquad() {
    ...
    setSaveError(null);
    setStep("squad");
    setNotice("Tim je ispražnjen. U bazu se ništa ne upisuje dok ne klikneš „Sačuvaj tim”.");
  }
  ...
  function handleAutoPick() {
    ...
    if (!result) {
      setSaveError(
        "Automatski izbor nije našao validan tim sa trenutnim igračima i budžetom — sastavi tim ručno."
      );
      return;
    }
    setSaveError(null);
  ...
  function handleLineupClick(p: SelectablePlayer) {
    ...
    if (captainId === leaving.id) {
      setCaptainId(null);
      setNotice("Kapiten je otišao na klupu — izaberi novog.");
    }
    if (viceCaptainId === leaving.id) {
      setViceCaptainId(null);
      setNotice("Vice-kapiten je otišao na klupu — izaberi novog.");
    }
    setSwapId(null);
  }
  ...
  async function handleSave() {
    ...
    if (data && data.ok === false) {
      setSaveError("Tim nije sačuvan — pokušaj ponovo.");
      setSaving(false);
      return;
    }
  ...
  return (
    <div className="bg-navy-800 rounded-xl ring-1 ring-black/25 px-4 py-3 mb-4">
      <div className="flex flex-wrap items-baseline gap-x-6 gap-y-1 text-sm">
        <span>
          <span className="text-slate-400">Igrača </span>
...
function LineupStep({
  ...
  return (
    <>
      ...
      <Bench note={`Formacija ${formationLabel}`}>{bench.map((p) => render(p, false))}</Bench>
    </>
  );
}
...
function SwapHelp({ swapPlayer }: { swapPlayer: SelectablePlayer | null }) {
  return (
    <div className="bg-navy-800 rounded-xl ring-1 ring-black/25 p-4 text-sm">
      <h3 className="font-display text-lg mb-2">Postava</h3>
      {swapPlayer ? (
        <p className="text-slate-300">
          Izabran je <strong>{playerFullName(swapPlayer)}</strong>. Klikni na igrača sa druge strane
          (teren ↔ klupa) da ih zameniš. Prigušeni igrači bi pokvarili formaciju.
        </p>
      ) : (
        <p className="text-slate-300">
          Klikni na igrača, pa na drugog sa suprotne strane, da ih zameniš mestima. Formacija mora
          ostati u granicama: 1 golman, 3–5 odbrana, 2–5 vezni, 1–3 napad.
        </p>
      )}
      <p className="text-slate-400 text-[13px] mt-3">
        Kapitena i vice-kapitena biraš dugmićima <strong>C</strong> i <strong>V</strong> ispod dresa.
        Kapiten donosi dvostruke poene, a vice ga menja ako ne odigra nijedan minut.
      </p>
    </div>
  );
}
