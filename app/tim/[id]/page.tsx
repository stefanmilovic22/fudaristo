...
export const metadata = { title: "Tim — Fudaristo" };
 ...
 */
export default async function TimPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <Shell owner={owner} rank={standing?.rank ?? null} gameweekNumber={null}>
        <p className="text-slate-400">
          Nijedno kolo još nije zaključano. Sastavi se javno prikazuju tek kad kolo bude
          obračunato — pre roka je tuđi tim tajna.
        </p>
      </Shell>
    );
  }
  ...
  if (!squadRows || squadRows.length === 0) {
    return (
      <Shell owner={owner} rank={standing?.rank ?? null} gameweekNumber={lastFinalized.number}>
        <p className="text-slate-400">Ovaj tim nije imao sastav u {lastFinalized.number}. kolu.</p>
      </Shell>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="mt-4">
        <h3 className="font-display text-sm uppercase tracking-wide text-slate-400 mb-2">Klupa</h3>
        <div className="flex gap-2 sm:gap-4 flex-wrap">
          {bench.map((e) => (
            <PlayerCard key={e.id} entry={e} muted />
          ))}
        </div>
      </div>
    </Shell>
 ...
 */
function PlayerCard({ entry, muted }: { entry: Entry; muted?: boolean }) {
  return (
    <Jersey
      color={entry.color}
      name={entry.lastName}
      ...
      flag={entry.autoSubbedIn ? "Auto-sub: ušao umesto igrača bez minuta" : null}
      dimmed={muted}
    />
  );
}
...
function Shell({
  ...
  const CHIP_LABEL: Record<string, string> = {
    ...
    favorite_club_x2: "Favorite Club x2",
    joker_1: "Joker #1",
    joker_2: "Joker #2",
  };
    ...
    <div>
      ...
      <div className="flex items-center gap-4 mt-3 mb-6">
        ...
        <div className="min-w-0">
          ...
          <p className="text-slate-400 text-sm">
            {rank !== null && <>{rank}. mesto · </>}
            {owner.total_points ?? 0} poena ukupno
            {owner.clubs?.name && <> · navija za {owner.clubs.name}</>}
          </p>
        </div>
      </div>
      ...
      {gameweekNumber !== null && (
        <div className="flex flex-wrap items-center gap-3 mb-4 text-sm">
          ...
          {transferCost ? (
            <span className="text-danger-400">{transferCost} za transfere</span>
          ) : null}
        </div>
      )}
