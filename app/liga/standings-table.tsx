    ...
    <div>
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        {myRank !== null && (
          ...
          >
            Moj tim je {myRank}. → pokaži
          </button>
        )}
        <span className="text-xs text-slate-500 ml-auto">
          {visible.length} od {rows.length} timova
        </span>
      </div>

      {gameweekNumber === null && (
        <p className="text-slate-400 text-sm mb-4 bg-navy-800 border border-navy-600 rounded-lg px-4 py-3">
          Nijedno kolo još nije zaključano, pa su svi poeni na nuli. Rang lista oživi čim admin
          obračuna prvo kolo.
        </p>
      )}
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="text-slate-400 text-xs uppercase tracking-wide">
              <th className="text-left font-semibold py-2 pl-3 pr-2 w-12">#</th>
              <th className="text-left font-semibold py-2 px-2">Tim</th>
              <th className="text-right font-semibold py-2 px-2 whitespace-nowrap">
                {gameweekNumber !== null ? `Kolo ${gameweekNumber}` : "Kolo"}
              </th>
              <th className="text-right font-semibold py-2 pl-2 pr-3">Ukupno</th>
            </tr>
          </thead>
          <tbody>
            {visible.map((row) => {
      ...
      {visible.length === 0 && (
        <p className="text-slate-400 text-sm py-6 text-center">
          Nijedan tim ne odgovara pretrazi „{query}”.
        </p>
      )}
    </div>
  );
