...
export function SettingsForm({
  ...
  async function save() {
    if (!teamName.trim()) {
      setError("Ime tima ne može biti prazno.");
      return;
    }
    setSaving(true);
    setError(null);
    ...
    if (updateError) {
      setError(
        updateError.code === "23505"
          ? "To ime tima je već zauzeto — izaberi drugo."
          : updateError.message
      );
      setSaving(false);
      return;
    ...
    <div className="bg-navy-800 rounded-xl ring-1 ring-black/25 p-5 flex flex-col gap-5">
      ...
      <label className="flex flex-col gap-1.5 text-sm">
        ...
        <span className="text-slate-400 text-xs">
          Ne utiče na fantasy pravila — služi samo za prikaz na tvom profilu.
        </span>
      </label>
      ...
      <div className="flex items-center gap-3">
        ...
        {saved && !dirty && <span className="text-gold-300 text-sm">Sačuvano.</span>}
      </div>
    </div>
  );
}
